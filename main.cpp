...
int main(int argc, char* argv[])	//получаем 3 аргумента: имя программы, имя файла и степень размытия
{
	if (argc < 3)
	{
	...
	if (argc > 3)	//так как аргументы разделяются пробелами, то имя "example image.bmp" будет считаться как 2 аргумента, поэтому их надо собрать в одну переменную
	{
